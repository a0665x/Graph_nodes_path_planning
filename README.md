...
1. 從「原始 Costmap · imgs/」選擇你的原始地圖，或匯入 PNG、JPEG、PGM。選單會遞迴列出 `imgs/` 與子目錄，完整保留相對路徑，不會改寫原圖。
...
首頁優先恢復上次成功選擇的 `imgs/` 原圖；第一次開啟預設使用 `imgs/map_1.png`，檔案不存在時才選擇另一張可用原圖。根目錄圖片優先，檔名依自然數字排序；例如 `imgs/map_7/map_7.png` 這類子目錄圖片也能直接選取，不必另外上傳。

瀏覽器允許儲存偏好時，重新整理會保留原圖選擇。範例圖放在獨立的「測試範例（非 imgs 原圖）」群組，只有使用者手動選擇才會載入，不會變成首頁預設或讀圖失敗時的替代圖。目錄為空或圖片不能讀取時會明確提示，保留重新選圖與匯入功能。讀取、選圖、規劃都不會覆寫 `imgs/` 的任何檔案。
